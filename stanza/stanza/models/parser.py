...
def parse_args(args=None):
    ...
    parser.add_argument('--wandb_project', type=str, default='uzbek-depparse', help='W&B project name.')
...
def train(args):
    ...
    if use_wandb:
        run_name = args.get('save_name', args['shorthand'] + '_parser')
        if run_name.endswith('.pt'):
            run_name = run_name[:-3]
        wandb.init(
            project=args.get('wandb_project', 'uzbek-depparse'),
            name=run_name,
            config={k: v for k, v in args.items() if isinstance(v, (str, int, float, bool, type(None)))},
            tags=['depparse', args.get('shorthand', ''), args.get('lang', '')],
        )
    ...
    with open(csv_log_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['step', 'train_loss', 'dev_score', 'uas', 'las', 'mlas', 'blex', 'lr', 'elapsed_sec'])
    ...
    while True:
        ...
        for i, batch in enumerate(train_batch):
            ...
            if global_step % args['eval_interval'] == 0:
                ...
                uas_f1 = evaluation['UAS'].f1 * 100
                las_f1 = evaluation['LAS'].f1 * 100
                mlas_f1 = evaluation['MLAS'].f1 * 100
                blex_f1 = evaluation['BLEX'].f1 * 100
                ...
                eval_uas_scores.append(uas_f1)
                eval_las_scores.append(las_f1)

                # Append to CSV
                with open(csv_log_file, 'a', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow([global_step, f'{train_loss:.6f}', f'{dev_score*100:.2f}',
                                     f'{uas_f1:.2f}', f'{las_f1:.2f}', f'{mlas_f1:.2f}',
                                     f'{blex_f1:.2f}', f'{current_lr:.6f}', f'{elapsed:.1f}'])
    ...
    if HAS_MATPLOTLIB and len(eval_steps) > 0:
        ...
        _save_parser_plots(plots_dir, run_label, eval_steps, eval_train_losses,
                           eval_dev_scores, eval_uas_scores, eval_las_scores,
    ...
    summary = {
        ...
        'final_uas': float(eval_uas_scores[-1]) if eval_uas_scores else 0,
        'final_las': float(eval_las_scores[-1]) if eval_las_scores else 0,
...
def _save_parser_plots(plots_dir, run_label, eval_steps, train_losses,
                       dev_scores, uas_scores, las_scores,
                       step_losses, lr_history):
    """Generate and save training curves for dependency parser."""
    # 1. Loss curve
    ...
    # 2. Dev accuracy curves (UAS, LAS)
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(eval_steps, uas_scores, 'g-o', markersize=3, label='UAS')
    ax.plot(eval_steps, las_scores, 'r-s', markersize=3, label='LAS')
    ax.set_xlabel('Step')
    ax.set_ylabel('Score (%%)')
    ax.set_title(f'Dev Parsing Scores — {run_label}')
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    ...
    ax2 = ax1.twinx()
    ax2.set_ylabel('Dev LAS (%%)', color=color_score)
    ax2.plot(eval_steps, las_scores, color=color_score, linestyle='-', marker='s', markersize=3, label='LAS')
